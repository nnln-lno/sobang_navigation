...
namespace navigation
{
  ...
  void Navigation::initAlignment(const sensor_msgs::msg::Imu::SharedPtr msg)
  {
    ...
    if (!do_align_)
    {
      ...
      if (alignment_count_ >= imu_rate * align_time_) // [HYPERPARAM] Align for the specified time duration
      {
        ...
        setState(Vec3d{0.0, 0.0, 0.0}, euler2quat(Vec3d{phi, theta, psi}), acc_mean, gyro_mean);
  ...
  void Navigation::publishDronePath(Vec3d position, Vec4d quaternion)
  {
    ...
    // if (ned_)
    // {
    //   pose.pose.position.x = position(0); // Forward Left Up to Forward Right Down conversion
    //   pose.pose.position.y = -position(1);
    //   pose.pose.position.z = -position(2);

    //   Vec3d conv_mat = Vec3d{quat2euler(quaternion)(0), -quat2euler(quaternion)(1), -quat2euler(quaternion)(2)}; // Rotation matrix for Forward Left Up to Forward Right Down conversion
    //   Vec4d conv_quat = euler2quat(conv_mat);

    //   pose.pose.orientation.x = conv_quat(1); //  Forward Left Up to Forward Right Down conversion
    //   pose.pose.orientation.y = conv_quat(2);
    //   pose.pose.orientation.z = conv_quat(3);
    //   pose.pose.orientation.w = conv_quat(0);
    // }
  ...
  void Navigation::param_setting()
  {
    ...
    Mat3d Cbr = euler2dcm(Vec3d({imu_R_radar_[0] * d2r, imu_R_radar_[1] * d2r, imu_R_radar_[2] * d2r  }));
    Vec3d tbr = Vec3d({imu_t_radar_[0], imu_t_radar_[1], imu_t_radar_[2]});
  }
}  // namespace navigation