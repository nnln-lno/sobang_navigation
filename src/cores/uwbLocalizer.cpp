...
namespace navigation
{
    ...
    void UWBLocalizer::setCurrentPose(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        ...
        if (imu_ned_)
        {
            ...
            Vec4d cur_quat = Vec4d{msg->pose.orientation.w, msg->pose.orientation.x, msg->pose.orientation.y, msg->pose.orientation.z};
            Vec3d conv_mat = Vec3d{quat2euler(cur_quat)(0), -quat2euler(cur_quat)(1), -quat2euler(cur_quat)(2)};
            Vec4d conv_quat = euler2quat(conv_mat);

            drone_att.x() = conv_quat.x();
            drone_att.y() = conv_quat.y();
            drone_att.z() = conv_quat.z();
            drone_att.w() = conv_quat.w();
        }    
        else{
            drone_pos.x() = msg->pose.position.x;
            drone_pos.y() = msg->pose.position.y;
